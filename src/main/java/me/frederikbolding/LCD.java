...
//CODE BASED ON THIS: https://www.raspberrypi.org/forums/viewtopic.php?p=423867#p423867
...
public class LCD {
    ...
    public static final int LED_YELLOW = LED_RED + LED_GREEN;
    public static final int LED_TEAL = LED_GREEN + LED_BLUE;
    public static final int LED_VIOLET = LED_RED + LED_BLUE;
    public static final int LED_WHITE = LED_RED + LED_GREEN + LED_BLUE;
    public static final int LED_ON = LED_RED + LED_GREEN + LED_BLUE;
    ...
    public LCD(int busNo, int address) throws IOException, InterruptedException {
        this.columns = 16;
        this.rows = 2;
        ...
        piPlateSetupRegisters[0] = (byte) (piPlateSetupRegisters[0] & ~(1 << 5));
        piPlateSetupRegisters[2] = (byte) (piPlateSetupRegisters[2] & ~(1 << 5));
        piPlateSetupRegisters[12] = (byte) (piPlateSetupRegisters[12] & ~(1 << 5));
    ...
    private void writeCmd(int cmd) {
        try {
            //System.out.println("Writing command: " + cmd);
            pollWait();
            int bitmask = portB & 0x01;   // Mask out PORTB LCD control bits
    ...
    public void shutdown() {
        ...
        setDisplay(false);
        setBacklight(false);
        setLedColor(LCD.LED_OFF);
    }

    byte[] _button_pins = new byte[]{0, 1, 2, 3, 4};
    boolean[] _button_state = new boolean[]{false, false, false, false, false};
    ...
    public boolean getButtonState(Button button) throws IOException {
        return !readBit(readButtons(), button.ordinal());
    }

    public boolean getButtonPressed(Button button) throws IOException {
        boolean state = getButtonState(button);
        if (state) {
            if (!_button_state[button.ordinal()]) {
                _button_state[button.ordinal()] = true;
                return true;
            }
        } else {
            _button_state[button.ordinal()] = false;
        }
        return false;
    }

    public boolean readBit(byte b, int bit) {
        return (b & (1 << bit)) != 0;
    }
